...
switch(cmd[|0]){
		...
		if(is_string(cmd[|1])){
			...
			switch(color){
				...
				_rainbow=0
					...
					_color_shadow[0]=make_color_rgb(127,127,127);
					_color_shadow[1]=make_color_rgb(127,127,127);
					_color_shadow[2]=make_color_rgb(127,127,127);
					_color_shadow[3]=make_color_rgb(127,127,127);
					break;
	...
	case "char_move":
		...
		break;
	case "head":
		if(is_real(cmd[|1])&&instance_exists(battle_enemy_test)){
			battle_enemy_test._face=cmd[|1]
		}
		break
	case "body":
		if(is_real(cmd[|1])&&instance_exists(battle_enemy_test)){
			battle_enemy_test._body=cmd[|1]
		}
		break
	case "legs":
	...
	case "black_cut":
		...
		fader.alarm[0]=cmd[|1];
	break;
	case "story_next_page":
		if(instance_exists(story)){
			if(story.page<=2){
				Anim_Create(story,"_alpha",0,0,1,-1,30);
				Anim_Create(story,"page",0,0,story.page,1,0,30);
				Anim_Create(story,"_alpha",0,0,0,1,30,30);
			}else{
				story.page+=1;
				Anim_Create(story,"shd",0,0,0,1,0);
				Anim_Create(story,"shd",0,0,1,-1,0,5);
			}
		}
	break;
	case "story_end":
