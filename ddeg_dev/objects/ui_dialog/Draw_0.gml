...
if (instance_exists(_inst) && _inst._paused && _choose_enable) {
	draw_sprite_ext(spr_battle_soul, 0, camera.x + x + 11.5 * (string_width(" ") * _inst._scale_x) + _choose_soul * (string_width("         "+_choice[0]) * _inst._scale_x) + 10, camera.y + _inst.y + 2.5 * ((string_height(" ") + 1) * _inst._scale_y), 1 / 2, 1 / 2, 90, c_red, 1)
}